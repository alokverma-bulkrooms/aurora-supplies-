...
const Navbar = () => {
  ...
  return (
    <nav className="bg-white/95 backdrop-blur-lg fixed w-full z-50 shadow-sm border-b border-gray-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center">
            <Link to="/" className="flex items-center space-x-3 group">
              ...
              <div className="flex flex-col">
                <span className="text-black text-2xl font-bold bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent">
                  Aurora
                </span>
                <span className="text-gray-600 text-sm font-medium -mt-1">Supplies</span>
              </div>
            </Link>
          </div>
        ...
        {isOpen && (
          <div className="md:hidden">
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-white/95 rounded-lg mt-2 border border-gray-200">
              {navigation.map((item) => (
                <Link
                  key={item.name}
                  to={item.href}
